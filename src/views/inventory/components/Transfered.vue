              ...
              <div class="table-container">
                <table class="table">
                  ...
                  <tbody v-if="filteredProducts.length > 0">
                    ...
                    <template v-for="(request, reqIndex) in filteredProducts">
                      <!-- Calculate rowspan for Req No and Email based on the number of products -->
                      <tr
                        v-for="(product, prodIndex) in request.products"
                        :key="product.productName + prodIndex"
                      >
                        <td
                          v-if="prodIndex === 0"
                        ...
                        <td style="width: 10%">{{ product.quantityTransferred }}</td>
                        <td style="width: 25%">
                          <div v-if="product.status === 0">
                            <i class="fas fa-arrow-alt-circle-right mr-1"></i>
                            <div style="color: green">Product Transfered</div>
                          </div>
                          <div v-else-if="product.status === 1" class="transferred">
                            <i class="fas fa-arrow-alt-circle-left mr-1"></i>
                            <div style="color: rgb(255, 0, 0)">Product Returned</div>
                          </div>
...
export default {
  ...
  methods: {
    async fetchProducts() {
      try {
        const response = await fetch("http://localhost:3000/gettransferedproduct");
        const data = await response.json();
        this.requests = data; // Assuming 'requests' is correctly populated from the backend
        this.filterData(); // Call a method to filter and set data for display
      } catch (error) {
